...
def get_pixels(i):
    ...
    return np.array([mask,np.array(to_conv), brdf, indx, indy], dtype = object)

randind = np.random.choice(np.unique(stm),40)

patches = (get_pixels(k) for k in randind)
...
parallel_rw_pkl(data, '2406test2_%i'%i, 'w')
