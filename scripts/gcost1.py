    ...
    for i, j in process:
        ...
        for ii,u in enumerate(ulist):
            ...
            if (mask.sum() >= 1800) and (mask.sum() < 3000):
                ...
                cost = []
                for xstd in np.arange(1,20,1):
                    for ystd in np.arange(1,20,1):
                        ...
                        m = mask[min(np.where(mask)[0]): max(np.where(mask)[0])+1, min(np.where(mask)[1]): max(np.where(mask)[1])+1]
                        gaus[~m] = 0
                        print np.nansum(gaus)
