                    ...
                    if xstd <=ystd:
                        for angle in xrange(30,160, 2):
                            ...
                            if kernel.sum() <= 0:
                                ...
                                if ker.shape[0]>to_conv.shape[0] or ker.shape[1]>to_conv.shape[1]:
                                    pass
                                else:
...
for i,l in enumerate(ite[:4]):
    patches = (get_pixels(k) for k in l)
    par = partial(shift, mode='gaus')
    pool = multiprocessing.Pool(processes=45)
    data = pool.map(par, patches)
    pool.close()
    pool.join()
    parallel_rw_pkl(data, '3006testgaus%02d'%i, 'w')
