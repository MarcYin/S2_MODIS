            ...
            if (mask.sum() >= 1800) and (mask.sum() < 4000):
                ...
                if (to_conv.shape[0]>100) & (to_conv.shape[1]>100) & (brdf!=np.nan):
                    ...
                    if gaus.sum() <= 0:
                        ...
                        saved.append([u, xstd, ystd, angle, conved, brdf, minsft[0], minsft[1]])

                        print 'solved one: ', saved[-1]
                else:
                    pass

            else:
                pass
    print 'finished this process %s %s'%(i,j)
    print saved[-1]
    return saved
...
gau = pool.map(par, pros)
pool.close()
pool.join()
parallel_rw_pkl(gau, 'psfb12_modis', 'w')

print 'lol finished psf12!!!!!'                                    