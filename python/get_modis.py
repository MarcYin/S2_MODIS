...
def get_modisfiles ( platform, product, year, tile, proxy, \
    ...
    for date in dates:
        ...
        #print urllib2.urlopen(req).readlines()
        try:
            html = urllib2.urlopen(req).readlines()
            for line in html:
                if line.find( tile ) >=0  and line.find(".hdf") >= 0 and \
                    ...
                    request = urllib2.Request("%s/%s/%s" % ( url, date, fname))
                    ...
                    if not os.path.exists ( os.path.join( out_dir, fname ) ):
                        ...
                        remote_file_size = int (the_remote_file.headers.dict['content-length'] )
                    ...
                    if download:
                        ...
                        with open ( os.path.join( out_dir, fname ), 'wb' ) \
                                as local_file_fp:
                            shutil.copyfileobj(opener.open(request), \
                                               local_file_fp)
                            if verbose:
                                LOG.info("Done!")
                    else:
